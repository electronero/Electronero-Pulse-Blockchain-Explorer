...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_short_str    = fmt::format("{:0.6f}", xmr_amount);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs_short"  , xmr_amount_to_str(xmr_inputs , "{:0.6f}")},
                {"sum_outputs_short" , xmr_amount_to_str(xmr_outputs, "{:0.6f}")},
                ...
                {"tx_size_short"     , fmt::format("{:0.6f}", tx_size)},
...
public:
    ...
    index2(uint64_t page_no = 0, bool refresh_page = false)
    {
        ...
        while (i >= start_height)
        {
            ...
            string blk_size_str = fmt::format("{:0.6f}", blk_size);
...
//        }
        ...
        mstch::map context {
                ...
                {"tx_fee"               , xmreg::xmr_amount_to_str(txd.fee, "{:0.6f}", true)},
                {"blk_timestamp"        , blk_timestamp},
        ...
        if (unsigned_tx_given)
        {
            ...
            if (r)
            {
                ...
                for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
                {
                    ...
                    tx_cd_data["timescales_scale"] = fmt::format("{:0.6f}",
...
private:
    ...
    construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
    {
        ...
        mstch::map context {
                ...
                {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
                {"tx_fee"                , xmreg::xmr_amount_to_str(txd.fee, "{:0.6f}", false)},
                {"payed_for_kB"          , fmt::format("{:0.6f}", payed_for_kB)},
                {"tx_version"            , static_cast<uint64_t>(txd.version)},
                {"blk_timestamp"         , blk_timestamp},
        ...
        if (detailed_view)
        {
            ...
            context["timescales_scale"] = fmt::format("{:0.6f}",
